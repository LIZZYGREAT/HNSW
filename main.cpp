...
hnsw::HNSW* build_index(float* base, size_t base_number, size_t vecdim)
{
    ...
    std::cerr << "Start building HNSW index (Linear Array Serial mode)..." << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "\n=== Starting HNSW Linear Array Serial Baseline Test ===\n";
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        for(size_t j = 0; j < k; ++j){
            uint32_t t = test_gt[j + i * test_gt_d];
            gtset.insert(t);
        }
    ...
    if (outfile.is_open()) {
        ...
        outfile << "Algorithm Name       : HNSW (Linear Array Backend)\n";
